        ...
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                <div class="modal-header">
                    ...
                    <h3 class="modal-title">Details Invoice Data</h3>
                </div>
                <form class="form-horizontal" role="form" method="POST" action="{{ url('invoice.delivery.submit') }}">
                    {{ csrf_field() }}
                ...
                <div class="modal-footer">
                    <button type="submit" class="btn btn-primary btn-reject pull-left">Submit</button>
                    {{--<button class="btn btn-primary btn-approve center-block" id="addcheckedfriends1">Reject</button>--}}
                    <button type="button" class="btn btn-danger pull-right" data-dismiss="modal">Cancel</button>
                    {{--<button class="btn btn-primary btn-approve pull-right">Submit</button>--}}
...
<script>
    ...
    $(function() {
        ...
        var table= $('#requisition-table').DataTable({
            ...
            autoWidth: false,
            responsive: true,
            ajax: 'invoice.delivery.data',
            columns: [
                { data: 'invoiceno', name: 'invoiceno' },
                { data: 'invoicedate', name: 'invoicedate' },
                { data: 'type', name: 'type', orderable: false, searchable: false },
                { data: 'relationship.name', name: 'relationship.name' },
                { data: 'relationship_id', name: 'relationship_id', visible: false },
...
//        $("body").on("click", "#requisition-table tbody tr", function (e) {
        $('#requisition-table').on('click', '.btn-detail', function (e) {
            ...
            var relationship_id = row.data().relationship_id;
...
//                    $(".rqdata").html("");
                    ...
                    $.each(data, function (i, item) {
                        trHTML += '<tr class="invoicedata"><td>' + item.refno + '</td><td>' +  item.item.name + '</td><td>' +  item.quantity + '</td><td>' +  item.item.unit_name + '</td>' +
                                '<td><input id="quantity" type="text" class="form-control" name="quantity[]" value="" required></td>' +
                                '<input id="invoiceno" name="invoiceno" type="hidden" value=' + item.refno + '>' +
                                '<input id="product_id" name="product_id[]" type="hidden" value=' + item.product_id + '>' +
                                '<input id="id" name="id[]" type="hidden" value=' + item.id + '>' +
                                '<input id="relationship_id" name="relationship_id" type="hidden" value=' + relationship_id + '></tr>';
                    });
